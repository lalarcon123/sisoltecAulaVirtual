<?php
  ...
  $comparar = true;
  $comparar_js = "<script type='text/javascript'>confirm('Desea anular su registro?')</script>";
...
if(isset($_POST['registro'])){
    if  ($existe['id']){
        ...
        if($comparar==$comparar_js){
            ...
            $update  = "update estudiante_curso set estado = 0 where id_user ='{$user}' and id_curso='{$id_curso}'";
            $result = $db->query($update);
        }
    } else {
      echo "<script type='text/javascript'>alert('Ud ya no esta registrado en este curso o el curso se encuentra Pagado')</script>";   
    }
}
if(isset($_POST['regresar'])) {
    redirect('detallesmecarts.php', false);
}
...
<body>
    <div class="contenedor">
            <form method="post" action="ConsultaCursoInfome.php?id=<?php echo (int)$curso['id'] ?>" >
                <section class="main">
                    <header>                        
                ...
                <div>
                  <button type="submit" name="registro" id="registro" class="btn btn-primary">Anular Registro</button>
                  <form method="post" action="detallesmecarts.php"><button type="submit" name="regresar" class="btn btn-danger">Regresar</button></form>                    
                </div>                
            </form>
            <footer>
            </footer>
