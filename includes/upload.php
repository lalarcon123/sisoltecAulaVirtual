...
class  Media {
  ...
  public $sitiosPath = SITE_ROOT.DS.'..'.DS.'uploads/QR';
  ...
  public function file_ext($filename){
     ...
     if(in_array($ext, $this->upload_extensions)){
       ...
       $this->errors[] = "extension erronea".$ext;
     }
  ...
  public function upload($file)
  {
    if(!$file || empty($file) || !is_array($file)):
      $this->errors[] = "Ningún archivo subido. xxxxx".$file;
      return false;
    elseif($file['error'] != 0):
 ...
 /*--------------------------------------------------------------*/
  public function process_media(){
    ...
    /*--------------------------------------------------------------*/
    /* Function for Process sitios image
    /*--------------------------------------------------------------*/
    public function process_sitios($id){
        ...
        if(!is_writable($this->sitiosPath)){
            $this->errors[] = $this->sitiosPath." Debe tener permisos de escritura";
            return false;
        }
        if(!$id){
            $this->errors[] = " No ha ingresado ningun sitio.";
            return false;
        }
        $ext = explode(".",$this->fileName);
        $new_name = randString(8).$id.'.' . end($ext);
        $this->fileName = $new_name;
        if($this->sitios_image_destroy($id))
        {
            if(move_uploaded_file($this->fileTempPath,$this->sitiosPath.'/'.$this->fileName))
            {

                if($this->update_sitiosImg($id)){
                    unset($this->fileTempPath);
                    return true;
                }
 ...
 /*--------------------------------------------------------------*/
  ...
  /*--------------------------------------------------------------*/
  /* Function for Update sitios image
  /*--------------------------------------------------------------*/
  private function update_sitiosImg($id){
        global $db;
        $sql = "UPDATE sitio SET";
        $sql .=" image_qr='{$db->escape($this->fileName)}'";
        $sql .=" WHERE id='{$db->escape($id)}'";
        $result = $db->query($sql);
        return ($result && $db->affected_rows() === 1 ? true : false);
 ...
 /*--------------------------------------------------------------*/
  ...
  /* Function for Delete old image
  /*--------------------------------------------------------------*/
  public function sitios_image_destroy($id){
     $image = find_by_id('sitio',$id);
     if($image['image_qr'] === 'no_image.jpg')
     {
          return true;
     } else {
          unlink($this->sitiosPath.'/'.$image['image_qr']);
          return true;
     }
