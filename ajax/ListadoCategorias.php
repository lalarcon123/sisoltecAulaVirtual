<?php
    include "../includes/config.php";//Contiene funcion que conecta a la base de datos

    //$query = "SELECT c.id,  convert(cast(convert(c.descripcion using latin1)  as binary) using utf8)  as descripcion, (case c.estado WHEN 1 THEN 'ACTIVO' ELSE 'INACTIVO' END) as estado FROM categoria c ";
    $query = "SELECT c.id,  
                     convert(cast(convert(c.descripcion using latin1)  as binary) using utf8)  as descripcion,  
                     (case c.estado WHEN 1 THEN 'ACTIVO' ELSE 'INACTIVO' END) as estado 
                FROM categoria c ";

    //echo $query;
    $resultado = mysqli_query($con,$query);

    if ($resultado){
        while($data = mysqli_fetch_assoc($resultado)){
            $arreglo["data"][] = array_map("utf8_encode",$data);
        }
        echo json_encode($arreglo);
    }
    mysqli_free_result($resultado);
    mysqli_close($con);
?>
