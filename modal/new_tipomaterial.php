...
if(isset($_POST['add'])) {
            ...
            if(empty($errors)) {
                ...
                $query = "insert into tipo_material (descripcion) values (";
                $query .=" '{$descripcion}'";
                $query .=")";

                $result   = $db->query($query);
                if($result && $db->affected_rows() === 1){
                    $query_aud  = "insert auditoria (nombre_tabla, id_usuario, accion) values ('tipo_material', ".$user.",'Ingreso')";
                    $result_aud = $db->query($query_aud);
                    if($result_aud && $db->affected_rows() === 1){
                            $session->msg('s',"Tipo Material ingresado exitosamente. ");
                    }
                } else {
...
?>
    ...
    <div> <!-- Modal -->
        ...
        <div class="panel-heading clearfix">
            <div class="pull-left">
                <button type="button" class="btn btn-default" data-toggle="modal" data-target=".bs-example-modal-lg-add"><i class="fa fa-plus-circle"></i> Agregar Tipo Material <span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>
                </button>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">Agregar Tipo Material</h4>
                </div>
