<?php
    ...
    $query .=" on p.id = r.id_proyecto inner join users re on re.id = p.responsable 
                                       inner join users u on u.id = r.responsable 
              WHERE ((r.id_proyecto = ";
    $query .= " '{$id}') AND ((r.responsable = '{$id_usuario}') 
                               OR ('{$id_usuario}' = '1') 
                               OR (EXISTS (SELECT * FROM proyectos p 
                                            WHERE p.id = r.id_proyecto 
                                              AND p.responsable = '{$id_usuario}')) 
                               OR (EXISTS (SELECT * FROM users u WHERE u.id = '{$id_usuario}' AND u.user_level = '4')))) 
                 AND r.fecha_inicio between  CAST('{$fecha_inicio}' AS DATE) AND CAST('{$fecha_fin}' AS DATE)";
