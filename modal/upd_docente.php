...
<?php
  if(isset($_POST['upd'])) {
    $req_fields = array('name','last_name','cedula','mail','phone','movil');
    ...
    if(empty($errors)){
         ...
         $sql = "UPDATE users SET name ='{$name}', last_name ='{$last_name}' , mail='{$mail}', phone='{$phone}', movil='{$movil}', ci='{$ci}'  WHERE id='{$db->escape($id)}'";
         //echo $sql;
         $result = $db->query($sql);
